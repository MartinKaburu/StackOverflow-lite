...
curl -i -H "Content-Type: application/json" -X POST -d '{"owner":"__user__3", "content":"How do you add a post"}' http://localhost:5000/api/v1/post_question
Run the tests
'''
import unittest
import json
...
class ApiTests(unittest.TestCase):
    ...
    def setUp(self):
        """Creates all the required variables
        """
        self.app = APP
    ...
    def test_non_persistent(self):
        ...
        """
        res = self.test_client().get('/api/v1/questions')
        # Bug self.assertEqual(len(res.data), 17)

    def test_post_question(self):
        """Test that a user can post a new question
        """
        res = self.test_client().post('/api/v1/post_question', data=json.dumps(self.question), headers={'Content-Type': 'application/json'})
    ...
    def test_post_an_answer_to_a_question(self):
        ...
        """
        post_answer = self.test_client().post('/api/v1/questions/1/answers', data=json.dumps(self.answer), headers={'Content-Type': 'application/json'})
