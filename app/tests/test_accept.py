...
class TestAccept(BaseTest):
    ...
    def test_accept_answer(self):
        ...
        accept_answer = self.test_client().post('/api/v1/accept/1', headers = head)
    ...
    def test_accept_answer_twice(self):
        ...
        self.assertEqual(accept_answer.status_code, 200)
        self.assertIn("Answer Accepted", accept_answer.data)
        accept_answer_twice = self.test_client().post('/api/v1/accept/1', headers =  head)
        self.assertEqual(accept_answer_twice.status_code, 200)
        accept_answer_twice = self.test_client().post('/api/v1/accept/1', headers =  head)
        self.assertEqual(accept_answer_twice.status_code, 200)
